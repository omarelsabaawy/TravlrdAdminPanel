...
const DeleteBusinessModal: React.FC<DeleteBusinessProps> = ({ isDeleteBusinessModalOpen, onClose, businessId, businessImage }) => {
  const [loading, setLoading] = useState<boolean>(false);
  const router = useRouter();
  const handleDeleteBusiness = async (event: any) => {
    event.preventDefault();
    try {
      setLoading(true);
      const response = await deleteBusiness(businessId, businessImage);  
      if (response) {
        setLoading(false);
        router.refresh();
  ...
  return (
    <>
      ...
      <div
        ...
        tabIndex={-1}
        aria-hidden="true"
        className={`${isDeleteBusinessModalOpen ? 'flex items-center justify-center' : 'hidden'
          ...
          <div className="relative p-4 w-full max-w-md max-h-full">
            <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
              <div className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <button type="button" onClick={onClose} className="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="deleteModal">
                ...
                <p className="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this Business?</p>
